...
def tbls(dbname):
    ...
    inputs = log_and_extract_input(dbs, dbname)
...
def resource_by_id(dbname, resource, primary_key):
    ...
    try:
        ...
        if request.method == 'GET':
            ...
            r_table = dta.get_rdb_table(resource, dbname)
            key = primary_key.split(_key_delimiter)
            newValue = context.get("body",None)
            res = r_table.update_by_key(key, newValue)
            rsp = Response(json.dumps(res), status=200, content_type="application/json")
            return rsp
...
def get_resource(dbname, resource_name):
    ...
    try:
        ...
        if request.method == 'GET':
            ...
            r_table = dta.get_rdb_table(resource_name, dbname)
            newcontent = context.get("body", None)
            res = r_table.insert(newcontent)
            rsp = Response(json.dumps(res), status=200, content_type="application/json")
            return rsp
